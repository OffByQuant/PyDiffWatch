...
def test_list_pending_prefers_stored_evidence_without_refetch(tmp_path, monkeypatch, scan_stub):
...
def test_list_pending_falls_back_to_refetch_when_no_stored_evidence(tmp_path, monkeypatch, scan_stub):
    cfg = Config(db_path=tmp_path / "db.sqlite", cache_dir=tmp_path / "c", lock_path=tmp_path / "l")
    conn, rid = _queue_suspicious(cfg, tmp_path)   # no evidence stored (release_id had none)
    conn.close()
    scan_stub.fetch(lambda cfg, rel: None)   # re-fetch path, unavailable
    items = orchestrator.list_pending(cfg)
...
def test_backfill_captures_evidence_for_existing_flagged_row(tmp_path, monkeypatch, scan_stub):
    cfg = Config(db_path=tmp_path / "db.sqlite", cache_dir=tmp_path / "c", lock_path=tmp_path / "l")
    conn = store.connect(cfg); store.init_schema(conn)
...
def test_backfill_reports_failure_when_package_pulled(tmp_path, monkeypatch, scan_stub):
...
def test_backfill_reconstructs_first_release_scan(tmp_path, monkeypatch, scan_stub):
...
def test_backfill_default_skips_benign_flagged_but_all_flag_includes_it(tmp_path, monkeypatch, scan_stub):
...
def test_backfill_skips_rows_with_evidence_or_no_rules(tmp_path, monkeypatch, scan_stub):
    cfg = Config(db_path=tmp_path / "db.sqlite", cache_dir=tmp_path / "c", lock_path=tmp_path / "l")
    conn = store.connect(cfg); store.init_schema(conn)
    ...
    conn.close()
    scan_stub.fetch(lambda cfg, rel: (_ for _ in ()).throw(AssertionError("should not fetch skipped rows")))
